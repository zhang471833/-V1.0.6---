...
namespace HookPlan
{
    class KeyboardHook
    {
        ...
        //声明扫描枪事件委托
        public delegate void BarcodeEventHandler(string BarCodes);
        //扫描枪事件
       ...
       private int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
       {
           ...
           if ((nCode >= 0) && (KeyDownEvent != null || KeyUpEvent != null || KeyPressEvent != null))
           {
               ...
               if (KeyDownEvent != null && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN))
               {
                   ...
                   KeyDownEvent(this, e);
                    Debug.WriteLine(MyKeyboardHookStruct.scanCode.ToString()+"KeyDown");
                    //转化键码
                    ...
                    {
                        Debug.WriteLine(((char)inBuffer[0]).ToString() + "-");
                        //记录按键可打印字符ASCII码32-127
                        if ((inBuffer[0]>=32)&&(inBuffer[0]<=127))
                        {
                            
                            if (barCode.BarCode.Length<1)//如果还没有存字符
                            {
                                //先存储第一个字符，并记录时间
                                barCode.BarCode += ((char)inBuffer[0]).ToString();
                                barCode.Time = DateTime.Now;
                            }
                            else //如果已经开始存字符
                            {
                                //如果距离上个字符少于30ms，可能是扫描枪数据
                                double dTime = DateTime.Now.Subtract(barCode.Time).TotalMilliseconds;
                                Debug.WriteLine("Timespan:" + dTime.ToString());
                                if (dTime<40)
                                {
                                    //存储字符，并记录时间
                                    barCode.BarCode += ((char)inBuffer[0]).ToString();
                           ...
                           // BarcodeEvent(((char)inBuffer[0]).ToString() + "-OK!");
                        }
        ...
        private void OnTimer(Object source, ElapsedEventArgs e)
        {
           ...
           ((System.Timers.Timer)source).Stop();
            ...
            //定时时间到了，看看数据长度够不够，如果够了就算是码，否则清除
            if (barCode.BarCode.Length>9)
            {
                BarcodeEvent(barCode.BarCode);
                //发出去后，清空
                barCode.BarCode = "";
            }
            else
